...
class EditTestimonalScreen extends StatefulWidget {
  const EditTestimonalScreen({Key? key}) : super(key: key);

  @override
  State<EditTestimonalScreen> createState() => _EditTestimonalScreenState();
}
...
class _EditTestimonalScreenState extends State<EditTestimonalScreen> {
  ...
  Future addPartnerLogo() async {
    ...
    var url = APIString.grobizBaseUrl + APIString.add_app_logo_list;
    log("step --------  2");
  ...
  Future editPartnerLogo({String? imageAutoId}) async {
    ...
    var url = APIString.grobizBaseUrl + APIString.edit_app_logo_list;
    ...
    try {
      ...
      request.fields["app_logo_auto_id"] = imageAutoId.toString();
    } catch (exception) {
      request.fields["images"] = '';
      log('pic not selected');
    }
  ...
  Future deletePartnerLogo({String? imageAutoId}) async {
    showLoadingDialog();
    final body = {
      "app_logo_auto_id": imageAutoId.toString(),
    };

    log(body.toString());
    var url = APIString.grobizBaseUrl + APIString.delete_app_logo_list;
    log(url);
